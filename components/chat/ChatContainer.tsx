...
const ChatContainer = ({ currentUser, userList }: ChatContainerProps) => {
      ...
      function (
        ...
        setConversationList((prev) => {
          ...
          if (!Array.isArray(data.user) || data.user.length < 2) {
            console.error("Invalid user array in conversation:", data);
            return prev;
          }

          const existingConversation = prev.find((conv) => conv.id === data.id);
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-[20rem] md:w-[25rem] lg:w-[30rem] h-[calc(100vh-4rem)] max-h-[36rem]">
        {conversation === null ? (
          <>
            ...
            <div className="flex flex-col gap-2 md:gap-3 p-2 md:p-3 overflow-y-auto h-[calc(100%-120px)] pb-4">
              {conversationList && conversationList.length > 0 ? (
                ...
                <div className="flex flex-col gap-2 justify-center items-center text-center h-full text-zinc-300">
