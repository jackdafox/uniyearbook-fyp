...
import AddSocialsDialog from "../dialogs/AddSocialsDialog";
...
const EditProfileForm = ({ user, faculty, major, batch }: EditProfileProps) => {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4 xl:max-w-full">
      <div className="flex justify-center w-full">
      <Avatar className="w-24 h-24 md:w-36 md:h-36 xl:w-40 xl:h-40 mb-6 md:mb-10">
        <AvatarImage src={profilePicture ? profilePicture : ""} />
        <AvatarFallback>{getInitials(user.first_name)}</AvatarFallback>
      </Avatar>
      </div>
      <Form {...form}>
      <form onSubmit={form.handleSubmit(processForm)} className="space-y-4 md:space-y-5">
        <FormField
        ...
        render={({ field }) => (
          ...
          <EventDialog
            className={field.value ? URL.createObjectURL(field.value) : ""}
            state={!!field.value}
          />
          </FormItem>
        )}
